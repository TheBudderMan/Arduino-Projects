...
void loop() {
  ...
  for (x = 5; x <= 127; x++) {
    display.drawLine(x , 31 - (data[x - 1]), x , 31 - (data[x]), WHITE);
  }
  // draw the axis, lables and tick marks
  for (int y = 0; y < 6; y++) {
    display.drawFastHLine(7, y * (24) / 5 + 4, 7, WHITE);
  }
  ...
  display.fillRect(0, 0,  127 , 2, BLACK);
  display.setTextColor(WHITE, BLACK);
  display.drawFastHLine(10, 0,  128 - 10, WHITE);
  display.drawFastHLine(10, 31,  128 - 10, WHITE);
  display.drawFastVLine(10, 0,  31, WHITE);
  display.setTextSize(1);
  display.setCursor(2, 3);
  //display.println("lulz-O-scope");
